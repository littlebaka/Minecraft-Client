// Decompiled by Jad v1.5.8g. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.kpdus.com/jad.html
// Decompiler options: packimports(3) braces deadcode fieldsfirst 

package net.minecraft.src;

import java.util.Random;

// Referenced classes of package net.minecraft.src:
//            WorldGenerator, World, Block, BlockLeaves, 
//            BlockGrass

public class WorldGenTrees extends WorldGenerator
{

    public WorldGenTrees(boolean flag)
    {
        super(flag);
    }

    public boolean func_516_a(World world, Random random, int i, int j, int k)
    {
        int l = random.nextInt(3) + 4;
        boolean flag = true;
        if(j < 1 || j + l + 1 > world.field_35472_c)
        {
            return false;
        }
        for(int i1 = j; i1 <= j + 1 + l; i1++)
        {
            byte byte0 = 1;
            if(i1 == j)
            {
                byte0 = 0;
            }
            if(i1 >= (j + 1 + l) - 2)
            {
                byte0 = 2;
            }
            for(int i2 = i - byte0; i2 <= i + byte0 && flag; i2++)
            {
                for(int l2 = k - byte0; l2 <= k + byte0 && flag; l2++)
                {
                    if(i1 >= 0 && i1 < world.field_35472_c)
                    {
                        int j3 = world.func_600_a(i2, i1, l2);
                        if(j3 != 0 && j3 != Block.field_384_L.field_376_bc)
                        {
                            flag = false;
                        }
                    } else
                    {
                        flag = false;
                    }
                }

            }

        }

        if(!flag)
        {
            return false;
        }
        int j1 = world.func_600_a(i, j - 1, k);
        if(j1 != Block.field_337_v.field_376_bc && j1 != Block.field_336_w.field_376_bc || j >= world.field_35472_c - l - 1)
        {
            return false;
        }
        world.func_634_a(i, j - 1, k, Block.field_336_w.field_376_bc);
        for(int k1 = (j - 3) + l; k1 <= j + l; k1++)
        {
            int j2 = k1 - (j + l);
            int i3 = 1 - j2 / 2;
            for(int k3 = i - i3; k3 <= i + i3; k3++)
            {
                int l3 = k3 - i;
                for(int i4 = k - i3; i4 <= k + i3; i4++)
                {
                    int j4 = i4 - k;
                    if((Math.abs(l3) != i3 || Math.abs(j4) != i3 || random.nextInt(2) != 0 && j2 != 0) && !Block.field_343_p[world.func_600_a(k3, k1, i4)])
                    {
                        func_41060_a(world, k3, k1, i4, Block.field_384_L.field_376_bc, 0);
                    }
                }

            }

        }

        for(int l1 = 0; l1 < l; l1++)
        {
            int k2 = world.func_600_a(i, j + l1, k);
            if(k2 == 0 || k2 == Block.field_384_L.field_376_bc)
            {
                func_41060_a(world, i, j + l1, k, Block.field_385_K.field_376_bc, 0);
            }
        }

        return true;
    }
}
